...
def llm(inp):
    if "请你根据患者信息诊断出所患疾病" in inp:
        return "无法确定"
    elif "Task: Please select" in inp:
        if "Input: 无法确定" in inp:
            return '{"item_id": "#2"}'
        else:
            return '{"item_id": "#1"}'
    elif "请你根据患者信息和诊断给出具体的治疗建议" in inp:
        return "无"
    elif "请你根据患者信息提取出所有症状" in inp:
        return '{"symptoms": ["夜盲", "视力差", "易摔跤", "自幼身材矮小", "卵巢和子宫发育异常", "幼稚子宫", "月经不规律"]}'
    elif "请你根据患者信息和每个症状对应的疾病" in inp:
        return "先天性子宫发育不全"
    elif "请你根据患者信息提取出患者的出生日期" in inp:
        return "1980-03-04"
    elif "请你根据患者信息提取出患者的身高体重" in inp:
...
def rag(inp):
    data = {
        "symptoms": [
            ...
            {
                "symptom": "幼稚子宫",
                "possible_diseases": ["先天性子宫发育不全", "内分泌紊乱", "染色体异常"],
            },
            {
                "symptom": "月经不规律",
                "possible_diseases": [
