...
class NodeShape:
    default = '{m}["{x}"]'
    LLMNode = '{m}["{x}<br><font size=2>[{llm}] {model}</font>"]'
    RAGNode = '{m}@{{shape: docs, label: "{x}"}}'
    # LoopNode = '{x}(("{x}"))'
    BranchNode = '{m}{{"{x}"}}'
    CodeNode = '{m}[/"{x}"/]'
    WebNode = '{m}@{{shape: procs, label: "{x}"}}'
    ValueNode = '{m}@{{shape: notch-rect, label: "{m}\\n{x}"}}'
    ExitNode = '{x}[["{x}"]]'
    FileNode = '{m}@{{shape: div-rect, label: "{x}"}}'
    # MCPNode = '{m}("{x}")'
    APINode = '{m}>"{x}"]'
    DatabaseNode = '{m}@{{shape: cyl, label: "{x}"}}'
    InputData = '{x}(["{x}"])'
    OutputData = '{x}(["{x}"])'
